...
int pm8901_dump_mpp(struct seq_file *m, int curr_len, char *gpio_buffer)
{
	static const char *ctype[] = { "d_in", "d_out", "bi_dir", "a_in",
		"a_out", "sink", "dtest_sink", "dtest_out" };
	struct pm8901_mpp_chip *mpp_chip =
		dev_get_drvdata(pm8901_mpp_chip.chip.dev);
	u8 type, state;
	const char *label;
	int i, len;
	char gpio_buf[128];
	char *title_msg = "---------- PMIC 8901 MPP ----------";
	...
	for (i = 0; i < PM8901_MPPS; i++) {
		...
		label = gpiochip_is_requested(&pm8901_mpp_chip.chip, i);
		type = (mpp_chip->ctrl[i] & PM8901_MPP_TYPE_MASK) >>
			PM8901_MPP_TYPE_SHIFT;
		state = pm8901_mpp_get(&pm8901_mpp_chip.chip, i);

		len += sprintf(gpio_buf + len, "GPIO[%2d]: ", i);
		len += sprintf(gpio_buf + len, "[TYPE]%10s, ", ctype[type]);
		len += sprintf(gpio_buf + len, "[VAL]%s, ",
			state? "HIGH" : " LOW");
		len += sprintf(gpio_buf + len, "[CTRL][0x%02x]",
			mpp_chip->ctrl[i]);
