...
#define	S5K3H1GX_TOTAL_STEPS_NEAR_TO_FAR			128
#else
#define	S5K3H1GX_TOTAL_STEPS_NEAR_TO_FAR			52
#endif
...
static struct region_params_t g_regions[] = {
	...
	/* Region 1 */
	{
		.step_bound = {S5K3H1GX_TOTAL_STEPS_NEAR_TO_FAR, 0},
		.code_per_step = 2,
	},
};
...
static uint16_t g_scenario[] = {
	...
	S5K3H1GX_TOTAL_STEPS_NEAR_TO_FAR,
};
	...
	struct msm_actuator_ctrl_t *a_ctrl)
{
	...
	if (s5k3h1gx_act_t.step_position_table) {
	...
	if (a_ctrl->step_position_table != NULL) {
		uint16_t i = 0;
		uint16_t s5k3h1gx_nl_region_boundary1 = 2;
		uint16_t s5k3h1gx_nl_region_code_per_step1 = 32;
...
#ifdef USE_RAWCHIP_AF
			if (s5k3h1gx_msm_actuator_info->use_rawchip_af)
	...
	void __user *argp)
{
	...
	return (int) msm_actuator_config(&s5k3h1gx_act_t,
		s5k3h1gx_msm_actuator_info, argp); /* HTC Angie 20111212 - Rawchip */
}
...
static int __init s5k3h1gx_i2c_add_driver(
	void)
{
	LINFO("%s called\n", __func__);
...
static int32_t s5k3h1gx_act_create_subdevice(
	void *board_info,
	void *sdev)
{
	LINFO("%s called\n", __func__);

	s5k3h1gx_msm_actuator_info = (struct msm_actuator_info *)board_info;
